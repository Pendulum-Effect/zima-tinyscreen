...
Hardware target: **Waveshare ESP32-S3-LCD-1.3** (square 240x240, ST7789V2,
QMI8658 6-axis IMU, **no touchscreen**). The display currently shows CPU
temperature only, full-screen, since this board has no touch input for
multi-screen navigation.

Pin mapping was extracted directly from Waveshare's schematic PDF and
cross-checked against known ESP32-S3 hardware facts, but has **not yet
been flash-tested against physical hardware**. If the screen stays blank
on first boot: double-check the backlight (GPIO20) is driving HIGH, and
that RST (GPIO42) is toggling low-then-high during `gfx->begin()`.
